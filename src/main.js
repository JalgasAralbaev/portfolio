...
const letters = Array.from(document.querySelectorAll("[data-key]"));
const specs = Array.from(document.querySelectorAll("[data-spec]"));
const textExample = document.getElementById('textExample');

const text = `В начале 13 в. в Центральной Азии возникло новое государство - Монгольская империя. Объединение монгольских племен в немалой степени было вызвано изменением климатических условий местности, где проживали монголы. 11 и 12 вв. были благоприятными для монголов. Длительный период влажных лет в восточной степи привел к тому, что умножились стада, а, следовательно, одна и та же территория могла прокормить больше людей. Произошло увеличение населения в Монголии. Однако в конце 12 в. климат стал постепенно меняться в сторону ухудшения, стал более засушливым. Кочевое скотоводство стало малопродуктивным, в степи стало много избыточного населения. Началась обычная в таких условиях борьба с соседями за пастбища, а также вторжения на земли соседей-земледельцев.
...
function keydownHandler(event) {
    ...
    const letter = letters.find((x) => x.dataset.key.includes(event.key));
...
function press(letter) {
    ...
    viewUpdate(mustLetter);

}

function viewUpdate(mustLetter) {
    const string = party.strings[party.currentStringIndex];


    const showedString = party.strings.slice(
        party.currentStringIndex,
    ...
    firstLine.append(
        ...
        ...string
            ...
            .map((letter, index, array) => {
                nextKey.textContent = `${array[0]}`;
                return nextKey;
            }),
        ...string
            .slice(party.currentPressedString)
            .split('')
            .map((letter) => {
                return `${letter}`;
            })
    ...
    for (let i = 1; i < showedString.length; i++) {
        ...
        line.append(
            ...showedString[i].split('').map((letter) => {
